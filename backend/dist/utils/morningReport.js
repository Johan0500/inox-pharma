...
exports.sendMorningConnectionReport = sendMorningConnectionReport;
exports.scheduleMorningReport = scheduleMorningReport;
...
// ── Excel ─────────────────────────────────────────────────────
async function generateExcel(labName, delegates, date) {
...
// ── HTML email admin ──────────────────────────────────────────
function generateHtml(name, labName, delegates, date) {
...
async function getDelegatesData(laboratoryId, startOfDay) {
...
// ── Fonction principale ────────────────────────────────────────
async function sendMorningConnectionReport() {
    try {
            ...
            <div style="background:#064e3b;padding:16px;border-radius:0 0 12px 12px;text-align:center">
              <p style="color:rgba(255,255,255,0.5);font-size:11px;margin:0">
                INOX PHARMA © ${today.getFullYear()}
                    ...
                    await sendEmail(u.email, `📊 INOX PHARMA — Rapport global ${dateStr} (${totalConn}/${totalAll} délégués)`, html, attachments);
                    console.log(`  ✅ Email Super Admin → ${u.email}`);
                }
                else if (u.role === "ADMIN") {
                    // ── Admin → seulement ses labos ───────────────────
                    const adminLabIds = u.adminLabs.map((al) => al.laboratoryId);
                    ...
                    for (const labId of adminLabIds) {
                        ...
                        if (!data)
                            continue;
                        const excel = await generateExcel(data.labName, data.delegates, today);
                        await sendEmail(u.email, `📊 INOX PHARMA — ${data.labName.toUpperCase()} — ${dateStr}`, generateHtml(u.firstName, data.labName, data.delegates, today), [{
                                filename: `${data.labName}_${today.toISOString().slice(0, 10)}.xlsx`,
                                content: excel,
                                type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            }]);
                ...
                console.error(`  ❌ Échec ${u.email}:`, emailErr?.response?.body?.errors || emailErr?.message || emailErr);
            }
        }
