...
function sumVentes(lines) {
    return lines.reduce((a, l) => a + GROSSISTES.reduce((b, g) => b + (l[`${g}Vente`] || 0), 0), 0);
}
function sumCA(lines) {
    return lines.reduce((a, l) => a + GROSSISTES.reduce((b, g) => b + (l[`${g}Vente`] || 0) * l.pght, 0), 0);
}
// ── Rapport courant ──────────────────────────────────────────
router.get("/current", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
// ── Rapport d'hier ───────────────────────────────────────────
router.get("/yesterday", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
router.patch("/line/:id", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
// ── Ajouter un produit ───────────────────────────────────────
router.post("/:id/line", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
// ── Supprimer une ligne ──────────────────────────────────────
router.delete("/line/:id", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
// ── Soumettre ────────────────────────────────────────────────
router.post("/:id/submit", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
        ...
        await Promise.all(superAdmins.map((sa) => (0, mailer_1.sendEmail)(sa.email, `📊 Nouveau rapport — ${report.laboratory.name.toUpperCase()}`, (0, mailer_1.reportSubmittedEmail)(adminName, report.laboratory.name, report.month, report.year, totalCA, totalVentes))));
        await (0, notifications_1.notifyAdmins)("📊 Nouveau rapport soumis", `${adminName} a soumis le rapport ${report.laboratory.name} ${report.month}/${report.year}`, "/dashboard");
...
router.post("/:id/reopen", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
        const report = await prisma.salesReport.update({
            where: { id: req.params.id },
...
// ── Export Excel ─────────────────────────────────────────────
router.get("/export/excel", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN", "SUPER_ADMIN"), async (req, res) => {
    try {
        const report = await prisma.salesReport.findFirst({
        ...
        if (!report)
            return res.status(404).json({ error: "Aucun rapport trouvé" });
        ...
        GROSSISTES.forEach((g) => headers.push(`${g.toUpperCase()} STOCK`, `${g.toUpperCase()} VENTE`, "S%", "VALEUR"));
        ...
        report.lines.forEach((l, idx) => {
            const row = [l.itemNumber, l.designation, l.pght];
            let totStock = 0;
            let totVente = 0;
...
router.get("/all", auth_1.authenticate, (0, auth_1.requireRole)("SUPER_ADMIN"), async (req, res) => {
    try {
...
router.get("/stats", auth_1.authenticate, (0, auth_1.requireRole)("SUPER_ADMIN"), async (req, res) => {
    try {
        const { month, year } = req.query;
        const where = { status: "SUBMITTED" };
        if (month)
        ...
        const stats = reports.map((r) => {
            const grossisteStats = GROSSISTES.map((g) => {
                const valStock = r.lines.reduce((a, l) => a + (l[`${g}Stock`] || 0) * l.pght, 0);
                const caRealise = r.lines.reduce((a, l) => a + (l[`${g}Vente`] || 0) * l.pght, 0);
            ...
            const totalStocks = r.lines.reduce((a, l) => a + GROSSISTES.reduce((b, g) => b + (l[`${g}Stock`] || 0), 0), 0);
            return {
                id: r.id,
...
// ─────────────────────────────────────────────────────────────────────────────
router.post("/target-ca", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/ca-dashboard", auth_1.authenticate, (0, auth_1.requireRole)("ADMIN"), async (req, res) => {
    try {
        const { month, year } = getMonthYear();
        const report = await prisma.salesReport.findFirst({
            where: { adminId: req.user.id, month, year },
