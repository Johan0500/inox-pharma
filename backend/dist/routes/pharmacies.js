...
router.get("/", auth_1.authenticate, async (req, res) => {
    try {
        const { search, grossiste, zone, ville, page = "1", limit = "50", } = req.query;
        // Filtre labo du super admin
        const labName = req.headers["x-lab"];
        const where = {}; // Suppression du filtre isActive
        if (search?.trim()) {
            where.OR = [
...
router.post("/create", auth_1.authenticate, (0, auth_1.requireRole)("SUPER_ADMIN", "ADMIN"), async (req, res) => {
    try {
        ...
        if (newGrossiste?.trim()) {
            ...
            if (!g)
                g = await prisma.grossiste.create({ data: { name: newGrossiste.trim() } });
            grossisteId = g.id;
        }
        ...
        if (err.code === "P2002")
            return res.status(409).json({ error: "Cette pharmacie existe déjà pour ce grossiste" });
        console.error(err);
        res.status(500).json({ error: "Erreur serveur" });
    }
});
