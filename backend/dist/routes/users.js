...
router.post("/", auth_1.authenticate, (0, auth_1.requireRole)("SUPER_ADMIN", "ADMIN"), async (req, res) => {
    try {
        ...
        const hashedPwd = await bcryptjs_1.default.hash(password, 12);
        ...
        const user = await prisma.user.create({
            data: {
                ...
                ...(role === "ADMIN" && labs?.length && {
                    adminLabs: {
                        create: await Promise.all(labs.map(async (labName) => {
                            const lab = await prisma.laboratory.findUnique({ where: { name: labName } });
                            if (!lab)
                                throw new Error(`Laboratoire "${labName}" non trouvé`);
