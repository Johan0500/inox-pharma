...
async function buildLabWhere(req) {
...
router.get("/history/:delegateId", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const where = { delegateId: req.params.delegateId };
        if (date) {
...
router.get("/history", auth_1.authenticate, (0, auth_1.requireRole)("SUPER_ADMIN", "ADMIN"), async (req, res) => {
    try {
        const { delegateId, date } = req.query;
        const labWhere = await buildLabWhere(req);
