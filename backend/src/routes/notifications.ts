...
function buildSubscriptionPayload(title: string, body: string, url: string) {
    ...
    subs.map(async (sub) => {
      try {
        await webpush.sendNotification(
          ...
          buildSubscriptionPayload(title, body, url),
        );
      } catch (err: any) {
...
export async function notifyAdmins(title: string, body: string, url = "/") {
  ...
  await Promise.allSettled(
    admins.map((admin) => sendPushNotification(admin.id, title, body, url)),
  );
}
