...
router.get("/", authenticate, async (req, res) => {
  try {
    const { specialty, group, laboratory } = req.query as any;
...
router.get("/specialties", authenticate, async (_req, res) => {
  try {
...
router.post("/", authenticate, requireRole("SUPER_ADMIN", "ADMIN"), async (req: AuthRequest, res) => {
  try {
    ...
    if (!specialty?.trim()) return res.status(400).json({ error: "La spécialité est obligatoire" });

    // Pour les admins, vérifier qu'ils n'ajoutent que pour leur labo
    const labName = laboratory?.trim() || "croient";
    if (req.user!.role === "ADMIN") {
      const allowed = (req.user!.labs || []).some(
        (l: string) => l.toLowerCase() === labName.toLowerCase()
      );
      if (!allowed) return res.status(403).json({ error: "Vous ne pouvez ajouter des produits que pour votre laboratoire" });
    }
    ...
    if (err.code === "P2002") return res.status(409).json({ error: "Ce produit existe déjà" });
    console.error(err);
