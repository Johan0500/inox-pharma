...
function getMonthBounds(year: number, month: string) {
...
router.get("/", authenticate, requireRole("SUPER_ADMIN", "ADMIN"), async (req: AuthRequest, res) => {
  try {
    ...
    const month           = (req.query.month as string) || defaults.month;
    const year            = parseInt(req.query.year as string) || defaults.year;
...
router.get("/:delegateId", authenticate, async (req, res) => {
  try {
    const defaults = getCurrentMonthYear();
    const month    = (req.query.month as string) || defaults.month;
    const year     = parseInt(req.query.year as string) || defaults.year;
