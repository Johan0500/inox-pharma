...
const LABS = [
  {
    ...
    color: "#065f46",
    light: "#d1fae5",
    desc:  "Médicaments génériques et spécialités",
  },
  {
    id:    "croient",
    name:  "CROIENT",
    ...
    color: "#1e40af",
    light: "#dbeafe",
    desc:  "Solutions thérapeutiques innovantes",
  },
];
...
export default function LabSelector({ onSelect }: Props) {
  ...
  const now      = new Date();
  const greeting = now.getHours() < 12 ? "Bonjour" :
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:800, margin:"0 auto", padding:"40px 20px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:48 }}>
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            ...
            <div>
              <h1 style={{ fontFamily:"Georgia,serif", fontSize:20, fontWeight:700, color:"white", margin:0, letterSpacing:2 }}>
                INOX PHARMA
              </h1>
        ...
        <div style={{ marginBottom:48 }}>
          ...
          <div style={{
            ...
            borderRadius:12, padding:"8px 16px", marginTop:16,
          }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:20, marginBottom:20 }}>
            ...
            return (
              ...
              >
                <div style={{
                  ...
                  background: isSel ? "rgba(255,255,255,0.2)" : lab.light,
                  display:"flex", alignItems:"center", justifyContent:"center",
                  boxShadow: isSel ? "0 4px 15px rgba(0,0,0,0.2)" : `0 4px 15px ${lab.color}30`,
                }}>
                  <span style={{ fontSize:28 }}>{lab.emoji}</span>
                </div>
                <h3 style={{
                  fontFamily:"Georgia,serif", fontSize:20, fontWeight:700, margin:"0 0 6px",
                  color: isSel ? "white" : lab.color, letterSpacing:1,
                }}>
                  {lab.name}
                </h3>
                <p style={{ fontSize:12, margin:0, lineHeight:1.5, color: isSel ? "rgba(255,255,255,0.8)" : "#6b7280" }}>
                  {lab.desc}
                </p>
                <div style={{
                  marginTop:16, display:"flex", alignItems:"center", gap:6,
                  color: isSel ? "rgba(255,255,255,0.9)" : lab.color,
                  fontSize:12, fontWeight:600,
                  opacity: isHov||isSel ? 1 : 0,
                  transform: isHov||isSel ? "translateX(0)" : "translateX(-8px)",
                  transition:"all 0.2s",
                }}>
          ...
          >
            ...
            <div style={{
              ...
              transform: hovered==="all" ? "translateX(0)" : "translateX(-8px)",
              transition:"all 0.2s",
            }}>
              <span>Accéder à la vue globale</span>
              <span>→</span>
            </div>
          </button>
        </div>
        ...
        <div style={{
          ...
          <p style={{ color:"rgba(255,255,255,0.75)", fontSize:13, margin:0, lineHeight:1.6 }}>
            En sélectionnant un laboratoire spécifique, vous accédez uniquement aux données de ce laboratoire.
            La <strong style={{ color:"white" }}>Vue Globale</strong> vous permet de voir les informations des deux laboratoires simultanément.
          </p>
        </div>
      </div>
    </div>
